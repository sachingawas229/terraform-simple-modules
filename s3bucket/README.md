# mcp-s3-bucket-module

This repo contains terraform  code with resources to deploy of s3 bucket.

## Deploy Example
<pre>
    <code>
module "s3-bucket" {
  source               = "git@github.com:nationwide-building-society/mcp-terraform-modules.git//s3-bucket"
  bucket               = var.user_list_s3_bucket_name
  force_destroy        = false
  ...
  server_side_encryption_configuration = {
    rule = {
      apply_server_side_encryption_by_default = {
        kms_master_key_id = var.nbs-kms-key-id
...
| <a name="input_attach_entire_bucket_metric"></a> [attach\_entire\_bucket\_metric](#input\_attach\_entire\_bucket\_metric) | Controls if a bucket metric configuration with no filter should be attached to the bucket. | `bool` | `false` | no |
